...
boss/elite/mobエージェント体制を、VS Code の最新 Custom Agents 機能（frontmatter: tools/agents/handoffs）+
Prompt Files + Subagents 機能を用いた正式なオーケストレーション基盤へ移行する。

これにより以下を実現する：

- bossがオーケストレーターとして、elite・mobをサブエージェントとして並列委任できる
...
- [ ] `.github/prompts/` に以下の Prompt Files が存在する
  ...
  - `review-request.prompt.md`: eliteにレビュー依頼するテンプレ
  - `report-done.prompt.md`: mobの完了報告テンプレ
...
|---|---|---|---|
| boss.agent.md 更新 | mob-1 | 現行ファイル + 本スペック | `.github/agents/boss.agent.md` |
| elite.agent.md 更新 | mob-2 | 現行ファイル + 本スペック | `.github/agents/elite.agent.md` |
| mob.agent.md 更新 | mob-3 | 現行ファイル + 本スペック | `.github/agents/mob.agent.md` |
| Prompt Files 作成 | mob-4 | 本スペック + USAGE.md テンプレ | `.github/prompts/*.prompt.md` |
| ARCHITECTURE.md 更新 | mob-5 | 現行ファイル + 本スペック | `docs/ARCHITECTURE.md` |
| USAGE.md 更新 | mob-6 | 現行ファイル + 本スペック | `docs/USAGE.md` |
| decisions.md 記録 | mob-7 | 本スペック | `docs/decisions.md` |
| dashboard.md 更新 | elite | 全成果 | `status/dashboard.md` |
| instructions 整合性確認 | mob-8 | `.github/instructions/*.instructions.md` | 更新パッチ（必要な場合のみ） |
