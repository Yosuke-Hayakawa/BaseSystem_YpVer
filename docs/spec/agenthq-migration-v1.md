...
Orchestrator/Coordinator/Workerエージェント体制を、VS Code の最新 Custom Agents 機能（frontmatter: tools/agents/handoffs）+
Prompt Files + Subagents 機能を用いた正式なオーケストレーション基盤へ移行する。

これにより以下を実現する：

- Orchestratorがオーケストレーターとして、Coordinator・Workerをサブエージェントとして並列委任できる
...
- SOLID（SRP/DIP）を維持する
- 既存の運用ルール（ロギング契約、output/ 制約、チームオーナー確認ゲート）を破壊しない
...
- [ ] `.github/prompts/` に以下の Prompt Files が存在する
  ...
  - `review-request.prompt.md`: Coordinatorにレビュー依頼するテンプレ
  - `report-done.prompt.md`: Workerの完了報告テンプレ
  - `escalate.prompt.md`: チームオーナーお伺いテンプレ
...
|---|---|---|---|
| orchestrator.agent.md 更新 | worker-1 | 現行ファイル + 本スペック | `.github/agents/orchestrator.agent.md` |
| coordinator.agent.md 更新 | worker-2 | 現行ファイル + 本スペック | `.github/agents/coordinator.agent.md` |
| worker.agent.md 更新 | worker-3 | 現行ファイル + 本スペック | `.github/agents/worker.agent.md` |
| Prompt Files 作成 | worker-4 | 本スペック + USAGE.md テンプレ | `.github/prompts/*.prompt.md` |
| ARCHITECTURE.md 更新 | worker-5 | 現行ファイル + 本スペック | `docs/ARCHITECTURE.md` |
| USAGE.md 更新 | worker-6 | 現行ファイル + 本スペック | `docs/USAGE.md` |
| decisions.md 記録 | worker-7 | 本スペック | `docs/decisions.md` |
| dashboard.md 更新 | coordinator | 全成果 | `status/dashboard.md` |
| instructions 整合性確認 | worker-8 | `.github/instructions/*.instructions.md` | 更新パッチ（必要な場合のみ） |
