...
- boss/elite/mobの指示書・定義を更新し、運用のブレと競合（同一ファイル編集）を減らす。
- bossが「重要判断」をユーザに確認してから進める仕組みを明文化する。
...
- [ ] `.github/agents/boss.agent.md` に「重要判断はユーザに確認する」ルールが明文化され、質問フォーマット（選択肢と推奨）を含む。
- [ ] `.github/agents/elite.agent.md` / `.github/agents/mob.agent.md` にも、判断が必要な場合のエスカレーション先（boss）とログ場所が明記される。
- [ ] `.github/instructions/*.instructions.md` に、以下が明記される。
  - [ ] どの役割がどのファイルへ何を書くか（ロギング契約）
  - [ ] 生成物は `output/` 配下に限定（例外も明記）
- [ ] `status/dashboard.md` に「🚨 要対応（ユーザ判断待ち）」のセクションが追加され、eliteが判断待ち事項を集約する運用が書かれている（bossはユーザ確認ゲートを担当）。
...
|---|---|---|---|
| エージェント定義（chatagent）更新 | mob | `.github/agents/*.agent.md` | 更新パッチ |
| 指示書（instructions）更新 | mob | `.github/instructions/*.instructions.md` | 更新パッチ |
| 運用Doc/ignore更新 | mob | `docs/USAGE.md`, `.gitignore`, `status/dashboard.md`, `docs/decisions.md` | 更新パッチ |
