...
- 参考リポ（multi-agent-shogun 等）から「話法」「上様お伺い（判断待ちの集約）」「報告フォーマット」を、当リポジトリ（VS Code + Markdown運用）に適合させて導入する。
- 将軍/家老/足軽のやり取りを、短く・再利用可能・レビュー可能な形に統一する。
- 足軽の作業から「skill_candidate（得意領域の候補）」をボトムアップで収集し、以後のタスク割当精度を上げる。
...
- 重要判断は上様（ユーザー）確認ゲートを必須とし、判断待ちは `status/dashboard.md` の「🚨 要対応」に集約する。
...
家老/足軽の「将軍（または dashboard 更新者）への報告」は、先頭に YAML を付ける。

必須フィールド：

- `role`: `shogun` / `karo` / `ashigaru-<n>`
- `topic`: 作業トピック
...
status: done
outputs:
  - output/ashigaru/<task>/result.md
summary: |
  - <1>
  - <2>
  - <3>
...
|---|---|---|---|
| 会話/報告プロトコルSpec追加 | ashigaru | `docs/spec/_template.md` | `docs/spec/agent-communication-v1.md` |
| 指示書へテンプレ反映 | ashigaru | `.github/instructions/*.instructions.md` | 更新パッチ |
| USAGE/dashboardへ反映 | ashigaru | `docs/USAGE.md`, `status/dashboard.md` | 更新パッチ |
| 採用判断を記録 | ashigaru | `docs/decisions.md` | 更新パッチ |
