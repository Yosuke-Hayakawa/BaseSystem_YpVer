...
VS Code と Copilot Chat（Agent モード）で「Phillips/Cole/Barnes/Volkova」の役割分担を使って評価業務を進めます。
...
|---|---|---|
| Phillips | `.github/instructions/orchestrator.instructions.md` | `.github/agents/phillips.agent.md` |
| Barnes | `.github/instructions/coordinator.instructions.md` | `.github/agents/barnes.agent.md` |
| Cole（汎用） | `.github/instructions/worker.instructions.md` | `.github/agents/cole.agent.md` |

Cole は汎用ワーカーで、Barnes がタスクごとに必要な数だけ起動します。各タスクのドメイン知識・出力フォーマットは以下のタスクテンプレートで渡されます：

| タスクテンプレート | instructions ファイル | 対応ステップ |
|---|---|---|
| 仕様解析 | `.github/instructions/spec-analyzer.instructions.md` | ステップ1 |
| VT環境 | `.github/instructions/vt-environment.instructions.md` | ステップ3 |
| テスト仕様書 | `.github/instructions/test-spec.instructions.md` | ステップ4 |
| テストケース | `.github/instructions/testcase.instructions.md` | ステップ5 |
| 結果解析 | `.github/instructions/result-analyzer.instructions.md` | ステップ7 |
| 報告書 | `.github/instructions/report-writer.instructions.md` | ステップ8・9 |
...
## ユーザの役割：Phillipsへの指示の出し方

ユーザは **手を動かす人ではなく、判断と方向付けを担当** します。
Phillipsがオーケストレーションを全て行うため、やることは「指示を出す → 判断を返す → 成果物を承認する」の3つだけです。

### 🚀 最短ルート（3ステップ）

```
Step 1: Copilot Chat で Phillips を選択し、評価業務の依頼を投げる
Step 2: Phillipsが「お伺い」してきたら A/B/C を選ぶ
Step 3: status/task.md を見て output/ 配下の成果物を承認する
```

### Step 1: Phillipsに依頼を投げる

**操作手順:**
1. VS Code で Copilot Chat を開く（`Ctrl+Shift+I` またはサイドバー）
2. チャットモードを **Agent** に切り替える
3. エージェントを **Phillips** に選択する
4. 業務別のテンプレート（下記）を参考に依頼を送信する

---

## 仕様のピンポイント質問（Phillipsを通さなくてOK）

「〇〇の信号が変化する条件は？」「△△端子の電圧閾値は？」など、**仕様書の特定箇所を調べたいだけ**の場合は Phillips を経由する必要はありません。Agent モードでエージェントを指定せずに直接質問してください。
...
仕様書の解析がまだ済んでおらず、**信号一覧の整理・サマリー生成まで含めて依頼したい**場合は Phillips に依頼してください。Phillips が内部で Cole（仕様解析テンプレート）に委任し、`output/` 配下に成果物を生成します。

| やりたいこと | 推奨 |
|---|---|
| 変換済みの仕様から特定の信号について質問 | エージェント指定なしで直接質問 |
| 仕様書を添付して特定の箇所を質問 | エージェント指定なしで直接質問 |
| 仕様書の全体解析・信号一覧の整理を含めて依頼 | Phillips に依頼 |
...
2. 重要判断は私に確認してください
3. タスク分解(Phillips)→実装(Cole)→検証(Barnes)→統合(Phillips)の流れで進めてください
```
...
## Step 2: Phillipsの「お伺い」に答える

Phillipsは重要判断のたびに「🚨 お伺い」として確認を求めてきます：
...
> Phillipsは回答が来るまで該当作業をブロックするので、早めに返すとスムーズです。

---

## Step 3: 成果物を確認・承認する

Phillipsが完了報告したら、`status/task.md` でステータスを確認し、`output/` 配下の成果物をレビューします。
...
その後、Phillips へ「`docs/spec/<変換後ファイル名>.md` を参照して…」と依頼するだけです。
...
| テーマ名 | Phillips 口調 | Barnes 口調 | Cole 口調 |
|---|---|---|---|
