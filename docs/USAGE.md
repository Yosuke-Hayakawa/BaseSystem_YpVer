...
将軍が全工程を完了すると、以下を提示します:
- **差分要約**: 何がどう変わったか（ファイル一覧）
- **影響範囲**: 既存コードへの影響
- **残課題**: 将来対応すべき事項
- **ビルド手順**: テストの実行方法

進捗の詳細は `status/dashboard.md` で確認できます。
...
```
AES-128/192/256 のECBモード暗号化・復号を実装してほしい。

■ 概要
- C言語でスクラッチ実装
- ビルド環境: GCC + CMake
- テスト: GoogleTest + NISTテストベクタ
...
1. まず仕様とACを見せてください
2. 重要判断は私に確認してください
3. タスク分解→並列実装→レビュー→統合で進めてください
```

→ 将軍の動き: 仕様作成 → ビルド方式のお伺い → 家老にタスク分解 → 足軽×6に並列委任 → 家老レビュー → 完了報告

#### 例2: 調査・比較の依頼

```
RTOS（FreeRTOS / Zephyr / ThreadX）の比較調査をしてほしい。
観点: リアルタイム性能、メモリフットプリント、ライセンス、エコシステム
結果は output/reports/ に比較表を出力してください。
```

→ 将軍の動き: 仕様作成 → 足軽×3に各RTOS調査を並列委任 → 家老が比較表を統合 → 推奨案をお伺い

#### 例3: 既存コードのリファクタリング

```
src/driver/ のHAL層をPort/Adapter構造にリファクタしてほしい。
既存テストが全て通ること。破壊的変更があれば事前に確認してください。
```

#### 例4: テスト追加

```
src/parser/ の境界値テストが不足している。
最低限: NULL入力、空文字、最大長、不正フォーマットのテストを追加してほしい。
```

### ❌ やらなくていいこと（将軍が自動でやる）

| あなたがやらなくていいこと | 将軍/家老/足軽が自動で行うこと |
|---|---|
| 仕様ファイルの作成・更新 | 将軍が `docs/spec/` に作成 |
| タスクの分割・担当決め | 家老が分解し `status/dashboard.md` に記録 |
| 進捗の更新 | 家老が dashboard に集約 |
| 設計判断の記録 | 将軍が `docs/decisions.md` に記録 |
| コード実装・テスト | 足軽が担当ファイルのみ実装 |
| レビュー | 家老がAC適合・SOLID・安全性をチェック |

### 🔍 進捗の確認方法

| 知りたいこと | 見る場所 |
|---|---|
| 全体の進捗・誰が何をしているか | `status/dashboard.md` |
| 判断待ちの事項 | `status/dashboard.md` の「🚨 要対応」セクション |
| 仕様（何を作るか） | `docs/spec/*.md` |
| なぜその設計にしたか | `docs/decisions.md` |
| 生成物（調査メモ、ビルド等） | `output/` |
...
> 指示テンプレートと操作手順は、本ファイル上部の「上様（ユーザ）の役割：将軍への指示の出し方」を参照してください。

## 将軍→家老→足軽の連携（ファイル運用・リファレンス）
