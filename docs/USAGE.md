...
| mob（汎用） | `.github/instructions/worker.instructions.md` | `.github/agents/mob.agent.md` |

mob は汎用ワーカーで、elite がタスクごとに必要な数だけ起動します。各タスクのドメイン知識・出力フォーマットは以下のタスクテンプレートで渡されます：
...
仕様書の解析がまだ済んでおらず、**信号一覧の整理・サマリー生成まで含めて依頼したい**場合は boss に依頼してください。boss が内部で mob（仕様解析テンプレート）に委任し、`output/` 配下に成果物を生成します。
