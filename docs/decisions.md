...
## Decision: Cole を固定役割から汎用プール方式に変更

- いつ：2026-04-20
- 何を：Cole〜6 の固定役割構成を廃止し、汎用 Cole プール方式に移行。Barnes がタスクごとに必要な数の Cole を起動し、タスクテンプレート（旧・専用 instructions）を渡して指示する構成に変更。
- なぜ：固定 Cole では同一ステップ内の並列処理ができず、仕様書が大量にある場合や NG が多発した場合にボトルネックになっていた。SKILL.md（task-decomposition, multi-agent-orchestration）の設計思想と整合させるため。
- 代替案：
  - A. 現行維持（固定 Cole〜6）→ 並列効率の問題が残る
  - B. ハイブリッド（専任 + 汎用補助）→ Cole の種類が増えて管理が複雑化
...
- 影響：Barnes の指示構成が「固定 Cole を指名」から「テンプレートを選んで Cole に渡す」に変わる。既存のタスクテンプレート（instructions ファイル）はそのまま再利用。
