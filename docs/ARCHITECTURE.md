...
- Race Director（race-director）：Spec と受け入れ条件（AC）を確定し、重要判断はチームオーナーに確認してから進める
...
| `/escalate` | チームオーナーお伺い | Race Director |
...
チームオーナーがやることは3つだけ。それ以外はRace Director/Pit Chief/Mechanicが進めます（詳細は `docs/USAGE.md`）。

1. 依頼を投げる（Copilot Chat で Race Director を選ぶ）
2. 重要判断に答える（Race Directorの「🚨 チームオーナーお伺い」へ A/B/C などで回答する）
...
- Race Director：方針・受け入れ条件（AC）・優先度と、チームオーナーお伺い（重要判断）を担当（dashboard は直接編集しない）
