...
- elite：Spec をタスクへ分解し、mobへ配布し、`status/dashboard.md` に進捗を集約（単一更新者）
- mob：自分の担当タスクだけ実行し、要点をeliteへ返す（必要なら `output/` に生成物）
...
- mob層は複数起動して並列対応する（担当を分ける）
- 競合回避：同じファイルを複数のmobが同時に編集しないよう、eliteが **ファイル単位**でタスクを切る
