...
| # | 業務ステップ | AI化難易度 | 担当エージェント | 備考 |
|---|---|---|---|---|
| 1 | 製品仕様理解 | ★★☆（中） | 仕様解析エージェント | 文書解析・要点抽出は得意領域 |
| 2 | ハードウェア準備 | ★★★（難） | スコープ外 | 物理作業のため自動化困難 |
| 3 | VTシステム環境作成 | ★★☆（中） | VT環境エージェント | DBCやCAPLのドラフト生成 |
| 4 | テスト仕様書作成 | ★☆☆（易） | テスト仕様書エージェント | テンプレートからの生成が得意 |
| 5 | テストケース作成 | ★★☆（中） | テストケースエージェント | 仕様書からテストケースへの変換 |
| 6 | テスト実行 | ★★★（難） | スコープ外 | CANoe/VTシステムの実機操作 |
| 7 | 結果解析 | ★★☆（中） | 結果解析エージェント | ログ解析・NG分類支援 |
| 8 | 懸念点確認シート記載 | ★☆☆（易） | 報告書エージェント | テキスト生成が得意領域 |
| 9 | 試験報告書発行 | ★☆☆（易） | 報告書エージェント | テンプレートからの報告書生成 |
...
│       ├── testcase.instructions.md          ← テストケースエージェント
│       ├── result-analyzer.instructions.md   ← 結果解析エージェント
│       └── report-writer.instructions.md     ← 報告書エージェント
│
├── docs/
│   ├── README.md
│   ├── USAGE.md
