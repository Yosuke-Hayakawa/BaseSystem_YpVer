...
オシロスコープの自動操作スクリプト・ノウハウ。

## 機器一覧

| 機種 | 用途 | 詳細 |
|------|------|------|
| テクトロニクス MSO22/MSO24 | LIN波形キャプチャ | [テクトロ_2シリーズ/README.md](テクトロ_2シリーズ/README.md) |
| 横河 DLM3054HD | 汎用波形取得 | [DLM3054/README.md](DLM3054/README.md) |

## フォルダ構成

```
波形取得/
├── テクトロ_2シリーズ/   # pyvisa制御、LINキャプチャスクリプト
│   ├── capture_screen.py
│   ├── capture_lin_ids.py
│   ├── capture_eig.py
│   ├── capture_igon_rest.py
│   └── 説明書/
├── DLM3054/              # tmctlLib制御、自動計測スクリプト
│   ├── lib/              # 横河TMCTLライブラリ+DLL
│   ├── 自動制御.md       # コマンドリファレンス（1033行）
│   ├── 説明書/           # マニュアルPDF
│   └── ソフト/           # ドライバ・ライブラリZIP
└── README.md             # このファイル
```
...
```
list_images("波形取得/画像ファイル")
view_image("波形取得/画像ファイル/xxx.png")
```

詳細は `mcp/README.md` を参照。
