...
### 3重チェック体制（運用）
- **実行者（mob）**: 数値・判定基準には必ず仕様書の参照箇所を併記する。不明な点はTODO記法で明示する
- **検証者（elite/レビュー担当）**: 成果物に「根拠なしの数値」がないかを重点的に検証する。見つけたら即REJECTして差し戻す
- **最終チェック（報告書チェック担当）**: 報告書内のTODOが残っていないか、推定値が推定と明記されているか、数値に仕様書参照が併記されているかを確認する
...
`status/dashboard.md` を複数のチャットセッションが同時に使う場合のルール。

- 各チャットセッションは **自分専用のセクション** にだけ書く
- 他のセッションのセクションは **絶対に触らない**
- セッション開始時に `## セッション名` の見出しを追加し、そこに書いていく
- 書く内容：やったこと（✅）、進行中のこと（🔄）、判断待ち（❓）
