...
このテンプレートを渡された Cole は、通信仕様書とビットアサイン表をもとに、CANoe/VTシステム環境構築のドラフトを生成することだけを行う。
...
- `output/signal_list.md`（仕様解析エージェント Cole が生成）
- 通信仕様書、ビットアサイン表
...
// テストケース関数の例（テストケースエージェントCole の出力を参照して実装すること）
...
- テストケースの詳細設計はテストケースエージェント（Cole）の担当
